  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (!authenticatedUser?.role) {
        throw new Error(
          language === 'EN'
            ? 'Login succeeded, but the account role could not be read.'
            : 'Login berhasil, tetapi data peran akun tidak terbaca.'
        );
      }
      ...
      <div className="flex min-h-screen items-center justify-center bg-[#030712] font-body text-slate-200">
        <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.5 }} className="flex flex-col items-center gap-4">
          ...
          <span className="font-mono text-sm tracking-widest text-slate-400">
            {language === 'EN' ? 'STARTING SYSTEM...' : 'MEMULAI SISTEM...'}
          </span>
        </motion.div>
      </div>
    );
      ...
      >
        ...
        <div className="mt-2 flex flex-col gap-5 border-t border-white/10 pt-6">
          <div className="flex items-center justify-center gap-2 text-slate-400">
            ...
            <p className="text-xs tracking-wide">{t('encryptedSession')}</p>
          </div>

          <div className="flex justify-center gap-6">
            <span className="cursor-pointer font-mono text-[10px] uppercase tracking-wider text-slate-500 transition-colors hover:text-white">
